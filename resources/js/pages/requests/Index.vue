...
import {
    supply as supplyItem,
    pickCommand as pickCommandItem,
} from '@/routes/request-items';
...
function checkLocation(itemId: string) {
    router.post(
        pickCommandItem.url({ requestItem: itemId }),
        {},
        {
            preserveScroll: true,
        },
...
function handleSupplyConfirmed(
    itemId: string,
    scannedPartNumber: string,
    qty: number,
) {
    // Remove from local state immediately for instant feedback
    const index = realtimeItemsRaw.value.findIndex(
        (item) => item.id === itemId,
    );
    if (index !== -1) {
    ...
    router.post(
        supplyItem.url({ requestItem: itemId }),
        {
...
function queueVoiceAnnouncement(
    item: RequestItem,
    isNewItem = false,
    isReminder = false,
) {
    ...
    if (voiceQueue.value.some((q) => q.item.id === item.id)) {
        ...
        if (item.is_urgent) {
            voiceQueue.value = voiceQueue.value.filter(
                (q) => q.item.id !== item.id,
            );
    ...
    if (voiceQueue.value.length >= MAX_QUEUE_SIZE) {
        ...
        const normalIndex = voiceQueue.value.findIndex(
            (q) => !q.item.is_urgent,
        );
        if (normalIndex !== -1) {
    ...
    if (isNewItem && item.is_urgent) {
        ...
        const firstNonUrgentIndex = voiceQueue.value.findIndex(
            (q) => !q.item.is_urgent,
        );
                    ...
                    >
                        ...
                        >
                            ...
                            <span
                                ...
                                class="absolute -top-1 -right-1 flex h-5 min-w-[1.25rem] items-center justify-center rounded-full bg-blue-500 px-1 text-xs font-semibold text-white shadow-md"
                            >
                                {{ voiceQueue.length }}
                            </span>
                        </Button>
                            ...
                            <CardContent class="p-4">
                                ...
                                >
                                    ...
                                    <div class="flex-shrink-0">
                                        ...
                                        <Badge
                                            v-else-if="
                                                getDelayMinutes(item.id) > 5
                                            "
                                            class="gap-1 bg-yellow-500 text-white hover:bg-yellow-600"
                                        >
                                            <AlertCircle class="h-3 w-3" />
                                            Waiting
                                        </Badge>
